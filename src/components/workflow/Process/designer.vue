  ...
  <div>
    ...
    <el-collapse v-model="activeName">
      ...
      <el-collapse-item name="common">
        <template slot="title"><i class="el-icon-info" /> 常规信息</template>
        <common-panel :id="elementId" />
      </el-collapse-item>

      <!--   任务信息     -->
      <el-collapse-item v-if="elementType.indexOf('Task') !== -1" name="Task">
        <template slot="title"><i class="el-icon-s-claim" /> 任务配置</template>
        <user-task-panel :id="elementId" />
      </el-collapse-item>

      <!--   表单     -->
      <el-collapse-item v-if="formVisible" name="form">
        <template slot="title"><i class="el-icon-s-order" /> 表单配置</template>
        <form-panel :id="elementId" />
      </el-collapse-item>

      <!--   执行监听器     -->
      <el-collapse-item name="executionListener">
        <template slot="title"><i class="el-icon-s-promotion" /> 执行监听器
      ...
      <!--   任务监听器     -->
      <el-collapse-item v-if="elementType === 'UserTask'" name="taskListener">
        <template slot="title"><i class="el-icon-s-flag" /> 任务监听器
      ...
      <!--   多实例     -->
      <el-collapse-item v-if="elementType.indexOf('Task') !== -1" name="multiInstance">
        <template slot="title"><i class="el-icon-s-grid" /> 多实例</template>
        <multi-instance :id="elementId" />
      </el-collapse-item>
      <!--   流转条件     -->
      <el-collapse-item v-if="conditionVisible" name="condition">
        <template slot="title"><i class="el-icon-share" /> 流转条件</template>
        <condition-panel :id="elementId" />
      </el-collapse-item>

      <!--   扩展属性     -->
      <el-collapse-item name="properties">
        <template slot="title"><i class="el-icon-circle-plus" /> 扩展属性</template>
